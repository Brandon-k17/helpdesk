...
class WeatherService:
    ...
    def __init__(self):
        # Using a free API key - you should replace this with your own
    ...
    def get_weather(self, city="Paris"):
        ...
        Returns weather data or None if error
        """
        if not REQUESTS_AVAILABLE:
            logger.warning("Requests library not available. Install with: pip install requests")
            return None
            
        # Clear any cached demo data when we have a real API key
        cache_key = f"weather_{city.lower()}"
        if not self.api_key.startswith('8c8c8c8c'):  # Not the placeholder key
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                # Return demo data when API key is invalid
                return {
                    'city': city,
                    'country': 'FR',
                    'temperature': 18,
