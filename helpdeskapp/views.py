...
from .forms import ConnexionForm, DemandeForm, DomaineForm, InscriptionForm
    ...
    if request.method == 'POST':
        ...
        try:
            utilisateur = Utilisateur.objects.create(
                ...
                adresse=adresse, email=email, password=password, role=role
            )
...
def add_demande(request):
    ...
    return render(request, 'demande/form_demande.html', {'form': form, 'titre': 'Nouvelle demande'})
...
def dashboard_admin_view(request):
    ...
    stats = {
        ...
        'admins': users.filter(role='administrateur').count(),
    }
